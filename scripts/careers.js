...
const resume_error = document.getElementById('resume-error');
...
function errorMessage() {
    ...
    role.addEventListener('input', (event) => {
        if (role.validity.patternMismatch) {
            role_error.textContent = "Please Enter Valid Role"
        } else {
...
form.addEventListener("submit", (e) => {
    ...
    resumeFormData.push(formEntries);
    form.reset();
    overlay.style.visibility = "hidden";
    overlay.style.opacity = 0;
    closeModal(modal);
